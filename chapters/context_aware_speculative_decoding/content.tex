...
In order to give some idea what a typical UZGentRAG prompt might look like, one example is given below. It always starts with a role description. Then the documents are given, split by ``---''. Finally, the question of the user is added.
...
\begin{figure}[h]
  ...
  \caption{Distribution of acceptance lengths with CASD augmenting EAGLE-2. The tokens that are generated by copying with CASD, are highlighted in blue, while the gray tokens were generated by EAGLE-2.}
